...
class DDPM(nn.Module):
    ...
    def get_autoDDPM_anomaly(self, inputs: torch.Tensor,
        ...
        # Handle both fixed threshold (float) and auto threshold (array)
        if self.masking_threshold >= 0:
            # Fixed threshold: broadcast to all images in batch
            masking_threshold = self.masking_threshold
            combined_mask_binary = torch.cat([torch.where(combined_mask[i] > masking_threshold, torch.ones_like(
                torch.unsqueeze(combined_mask[i],0)), torch.zeros_like(combined_mask[i]))
                                              for i in range(combined_mask.shape[0])], dim=0)
        else:
            # Auto threshold: compute per-image 95th percentile
            masking_threshold = torch.tensor(np.asarray([(
                np.percentile(combined_mask[i].cpu().detach().numpy(), 95)) for i in range(combined_mask.shape[0])]).clip(0, 1))
